...
public class DoubleJumpHandler {
  ...
  static void onJump(LivingJumpEvent event) {
    ...
    if (living.onGround() || living.verticalCollision) {
      living.getCapability(PersistentDataCapability.CAPABILITY).remove(JUMPS);
    }
  }
  ...
  static void onLand(LivingFallEvent event) {
    event.getEntity().getCapability(PersistentDataCapability.CAPABILITY).remove(JUMPS);
  }
  ...
  public static boolean extraJump(Player entity) {
    ...
    if (!entity.onGround() && !entity.onClimbable() && !entity.isInWaterOrBubble()) {
      // determine max jumps
      int extraJumps = Mth.floor(entity.getAttributeValue(TinkerAttributes.JUMP_COUNT.getDelegate())) - 1;
      if (extraJumps > 0) {
