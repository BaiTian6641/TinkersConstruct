...
public class CombatFishingHookRenderer extends EntityRenderer<CombatFishingHook> {
  ...
  public void render(CombatFishingHook hook, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
    ...
    if (player != null) {
      ...
      texture.vertex(consumer, lastPose, pose, bobberLight, 0f, 0, 0, 1);
      texture.vertex(consumer, lastPose, pose, bobberLight, 1f, 0, 1, 1);
      texture.vertex(consumer, lastPose, pose, bobberLight, 1f, 1, 1, 0);
      texture.vertex(consumer, lastPose, pose, bobberLight, 0f, 1, 0, 0);
      poseStack.popPose();
  ...
  private static void stringVertex(float x, float y, float z, VertexConsumer consumer, PoseStack.Pose pose, float scale, float nextScale) {
    float lineX = x * scale;
    float lineY = y * (scale * scale + scale) * 0.5F + 0.25F;
    float lineZ = z * scale;
    float lineNextX = x * nextScale - lineX;
    float lineNextY = y * (nextScale * nextScale + nextScale) * 0.5F + 0.25F - lineY;
    float lineNextZ = z * nextScale - lineZ;
    float length = Mth.sqrt(lineNextX * lineNextX + lineNextY * lineNextY + lineNextZ * lineNextZ);
    if (length > 0.0F) {
      lineNextX /= length;
      lineNextY /= length;
      lineNextZ /= length;
    ...
    consumer.addVertex(pose.pose(), lineX, lineY, lineZ)
            .setColor(0, 0, 0, 255)
            .setNormal(pose, lineNextX, lineNextY, lineNextZ);
  }
  ...
  private record MaterialTexture(RenderType texture, int luminosity, int alpha, int red, int green, int blue) {
    ...
    public void vertex(VertexConsumer consumer, PoseStack.Pose poseStack, Matrix4f pose, int lightmap, float pX, int pY, int pU, int pV) {
      consumer.addVertex(pose, pX - 0.5f, pY - 0.5f, 0f)
              .setColor(red, green, blue, alpha)
              .setUv(pU, pV)
              .setOverlay(OverlayTexture.NO_OVERLAY)
              .setLight(lightmap)
              .setNormal(poseStack, 0.0F, 1.0F, 0.0F);
    }
  }
}
