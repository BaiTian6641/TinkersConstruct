...
public class AutosmeltModule implements ModifierModule, ProcessLootModifierHook {
  ...
  private final Cache<Item, Optional<RecipeHolder<? extends AbstractCookingRecipe>>> recipeCache = CacheBuilder
    .newBuilder()
    .maximumSize(64)
    .build();
   ...
   */
  private Optional<RecipeHolder<? extends AbstractCookingRecipe>> findRecipe(ItemStack stack, Level world) {
    SingleRecipeInput input = new SingleRecipeInput(stack);
    // try each recipe type to see if we have a recipe for any of them
    Optional<RecipeHolder<? extends AbstractCookingRecipe>> recipe = Optional.empty();
    for (RecipeType<? extends AbstractCookingRecipe> recipeType : recipeTypes) {
      @SuppressWarnings("unchecked")
      Optional<RecipeHolder<? extends AbstractCookingRecipe>> found = (Optional<RecipeHolder<? extends AbstractCookingRecipe>>)(Optional<?>)world.getRecipeManager().getRecipeFor(recipeType, input, world);
      recipe = found;
      if (recipe.isPresent()) {
        break;
      }
  ...
  private ItemStack smeltItem(ItemStack stack, Level world) {
    ...
    RecipeHolder<? extends AbstractCookingRecipe> recipe = findCachedRecipe(stack, world);
    if (recipe != null) {
      // fetch recipe result, may be input sensitive
      ItemStack output = recipe.value().assemble(new SingleRecipeInput(stack), world.registryAccess());
