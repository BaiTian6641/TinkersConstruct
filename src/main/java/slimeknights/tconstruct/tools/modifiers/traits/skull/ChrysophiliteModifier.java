...
public class ChrysophiliteModifier extends NoLevelsModifier implements EquipmentChangeModifierHook {
  ...
  public void onEquipmentChange(IToolStackView tool, ModifierEntry modifier, EquipmentChangeContext context, EquipmentSlot slotType) {
    ...
    if (slotType == EquipmentSlot.HEAD && isArmorSlot(changed)) {
    ...
    if (source != null) {
      ...
      if (gold > 0) {
        ...
        for (EquipmentSlot slot : EquipmentSlot.values()) {
          ...
          boolean hasVanishingCurse = stack.has(DataComponents.ENCHANTMENTS)
            && EnchantmentHelper.getTagEnchantmentLevel(
              target.level().registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(Enchantments.VANISHING_CURSE),
              stack) > 0;
          if (!stack.isEmpty() && !hasVanishingCurse && stack.makesPiglinsNeutral(target) && random.nextFloat() < extraChance) {
  ...
  private static boolean isArmorSlot(EquipmentSlot slotType) {
    return slotType == EquipmentSlot.HEAD || slotType == EquipmentSlot.CHEST || slotType == EquipmentSlot.LEGS || slotType == EquipmentSlot.FEET;
  }
}
