...
public class ModifierRecipeProvider extends BaseRecipeProvider {
  public ModifierRecipeProvider(PackOutput packOutput, CompletableFuture<HolderLookup.Provider> lookupProvider) {
  ...
  private void addModifierRecipes(RecipeOutput consumer) {
    ...
    ModifierRecipeBuilder.modifier(ModifierIds.harmonious)
                         .setTools(TinkerTags.Items.BONUS_SLOTS)
                         .addInput(ItemTags.create(ResourceLocation.withDefaultNamespace("music_discs")))
                         .setMaxLevel(1)
    ...
    SizedIngredient standardSkulls = SizedIngredient.of(DifferenceIngredient.of(Ingredient.of(TinkerTags.Items.SKULLS), Ingredient.of(Items.DRAGON_HEAD)));
  ...
  /** Overload for typed modifier IDs. */
  private ResourceLocation prefix(ModifierId id, String folder) {
    return prefix(id.getLocation(), folder);
  }

  /** Overload for typed modifier IDs. */
  private ResourceLocation wrap(ModifierId id, String prefix, String suffix) {
    return wrap(id.getLocation(), prefix, suffix);
  }
}
