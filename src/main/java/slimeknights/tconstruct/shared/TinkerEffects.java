...
public class TinkerEffects extends TinkerModule {
  ...
  // slimy potions
  public static final DeferredHolder<MobEffect, TinkerEffect> experienced = MOB_EFFECTS.register("experienced", () -> new TinkerEffect(MobEffectCategory.BENEFICIAL, 0x82c873, true).addAttributeModifier(TinkerAttributes.EXPERIENCE_MULTIPLIER.getDelegate(), "ccffb654-9988-451e-9539-f74934274df1", 0.25f, Operation.ADD_MULTIPLIED_BASE));
  public static final DeferredHolder<MobEffect, TinkerEffect> ricochet = MOB_EFFECTS.register("ricochet", () -> new TinkerEffect(MobEffectCategory.NEUTRAL, 0x01cbcd, true).addAttributeModifier(TinkerAttributes.KNOCKBACK_MULTIPLIER.getDelegate(), "58a4bc13-366f-4f76-82f5-705451498c24", 0.5f, Operation.ADD_MULTIPLIED_BASE));
  ...
  // slimy cakes
  public static final DeferredHolder<MobEffect, TinkerEffect> bouncy = MOB_EFFECTS.register("bouncy", () -> new TinkerEffect(MobEffectCategory.BENEFICIAL, 0x71AC63, true).addAttributeModifier(TinkerAttributes.BOUNCY.getDelegate(), "5de036ed-bc47-4965-9348-64c3ab5c8ae8", 1, Operation.ADD_VALUE));
  public static final DeferredHolder<MobEffect, TinkerEffect> doubleJump = MOB_EFFECTS.register("double_jump", () -> new TinkerEffect(MobEffectCategory.BENEFICIAL, 0xA99B87, true).addAttributeModifier(TinkerAttributes.JUMP_COUNT.getDelegate(), "9863601a-9d4a-4708-b348-4bf9fe6c0bbd", 1, Operation.ADD_VALUE));
  ...
  public static final DeferredHolder<MobEffect, SelfDestructiveEffect> selfDestructing = MOB_EFFECTS.register("self_destructing", SelfDestructiveEffect::new);
  ...
  private static void brewing(RegisterBrewingRecipesEvent event, EnumObject<PotionType,Potion> potion, net.minecraft.core.Holder<Potion> base, Item ingredient) {
    var builder = event.getBuilder();
    var normal = BuiltInRegistries.POTION.wrapAsHolder(potion.get(PotionType.NORMAL));
  ...
  private static void registerBrewingRecipes(RegisterBrewingRecipesEvent event) {
    brewing(event, experiencedPotion, Potions.AWKWARD, TinkerWorld.congealedSlime.get(SlimeType.EARTH).asItem());
    brewing(event, ricochetPotion, Potions.AWKWARD, TinkerWorld.congealedSlime.get(SlimeType.SKY).asItem());
    brewing(event, levitationPotion, Potions.AWKWARD, TinkerWorld.congealedSlime.get(SlimeType.ICHOR).asItem());
    brewing(event, enderferencePotion, Potions.AWKWARD, TinkerWorld.congealedSlime.get(SlimeType.ENDER).asItem());
  }
