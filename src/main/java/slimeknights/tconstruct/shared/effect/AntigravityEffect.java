...
public class AntigravityEffect extends TinkerEffect {
    ...
    if (living.getAttributeValue(Attributes.GRAVITY) < 0) {
      ...
      if (living.verticalCollision && !living.verticalCollisionBelow) {
        if (lastVelocity.twoTicks > 0.1f) {
          ...
          float height = (float)(lastVelocity.twoTicks * 10 - 3 - living.getAttributeValue(TinkerAttributes.SAFE_FALL_DISTANCE.getDelegate()) - TinkerEffect.getLevel(living, MobEffects.JUMP));
          if (height > 0.0F) {
      ...
      if (living.onClimbable()) {
        ...
        if (living.horizontalCollision || living.zza > 0 || living.xxa != 0) {
          y = -0.2;
        }
  ...
  /** Clears cached motion data once the effect is removed. */
  private void onEffectRemoved(MobEffectEvent.Remove event) {
    if (event.getEffect().equals(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(this))) {
