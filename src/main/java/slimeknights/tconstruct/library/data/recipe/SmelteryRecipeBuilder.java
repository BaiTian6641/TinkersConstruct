...
public class SmelteryRecipeBuilder {
  ...
  private void oreMelting(float scale, String tagPrefix, @Nullable TagKey<Item> size, float factor, String output, boolean forceOptional) {
    ...
    if (oreByproducts.length == 0) {
      ...
      // ConditionalRecipe was removed in modern NeoForge; keep deterministic output and only include guaranteed byproducts.
      boolean saved = false;
  ...
  public SmelteryRecipeBuilder itemMelting(float scale, String domain, String path, boolean damagable) {
    itemMelting(scale, domain + '_' + path, (float)Math.sqrt(scale), ResourceLocation.parse(domain + ":" + path), damagable);
    return this;
  }
  ...
  public SmelteryRecipeBuilder metalMelting(float scale, String domain, String path, boolean damagable) {
    itemMelting(scale, domain + '_' + path, (float)Math.sqrt(scale), ResourceLocation.parse(domain + ":" + name.getPath() + '_' + path), damagable);
    return this;
  }
  ...
  public SmelteryRecipeBuilder oreberry() {
    ...
    itemMelting(1/9f, "oreberry", 1 / 3f, ResourceLocation.parse("oreberriesreplanted:" + name.getPath() + "_oreberry"), false);
    return this;
  }
