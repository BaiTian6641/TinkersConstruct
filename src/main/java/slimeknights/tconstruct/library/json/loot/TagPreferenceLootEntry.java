...
public class TagPreferenceLootEntry extends LootPoolSingletonContainer {
  public static final MapCodec<TagPreferenceLootEntry> CODEC = RecordCodecBuilder.mapCodec(instance -> singletonFields(instance).and(
    ResourceLocation.CODEC.fieldOf("tag").forGetter(entry -> entry.tag.location())
  ).apply(instance, (weight, quality, conditions, functions, tagId) -> {
    ...
    return new TagPreferenceLootEntry(TagKey.create(Registries.ITEM, tagId), weight, quality, conditions, functions);
  }));
