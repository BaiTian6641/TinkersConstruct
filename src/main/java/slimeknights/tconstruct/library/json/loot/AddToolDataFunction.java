...
public class AddToolDataFunction extends LootItemConditionalFunction {
  ...
  public static final MapCodec<AddToolDataFunction> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
    ...
    RANDOM_MATERIAL_CODEC.listOf().optionalFieldOf("materials", List.of()).forGetter(function -> function.materials)
  ...
  protected AddToolDataFunction(List<LootItemCondition> conditionsIn, float damage, List<RandomMaterial> materials) {
    ...
    if (damage < 0 || damage > 1) {
      throw new IllegalArgumentException("damage_percent must be between 0 and 1, given " + damage);
    }
    this.damage = damage;
  ...
  public static class Builder extends LootItemConditionalFunction.Builder<AddToolDataFunction.Builder> {
    private final ImmutableList.Builder<RandomMaterial> materialList = ImmutableList.builder();
    ...
    public LootItemFunction build() {
      return new AddToolDataFunction(List.copyOf(getConditions()), damage, materialList.build());
    }
  }
}
