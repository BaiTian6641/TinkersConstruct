...
public class ToolPartLootEntry extends LootPoolSingletonContainer {
  private static final Codec<RandomMaterial> RANDOM_MATERIAL_CODEC = Codec.PASSTHROUGH.flatXmap(
    dynamic -> {
      JsonElement json = (JsonElement) dynamic.convert(JsonOps.INSTANCE).getValue();
      if (!json.isJsonObject()) {
        return DataResult.error(() -> "Expected JSON object for random material");
      }
      try {
        return DataResult.success(RandomMaterial.LOADER.convert(json, "material"));
      } catch (RuntimeException e) {
        return DataResult.error(e::getMessage);
      }
    },
    material -> DataResult.success(new Dynamic<>(JsonOps.INSTANCE, RandomMaterial.LOADER.serialize(material)))
  );

  public static final MapCodec<ToolPartLootEntry> CODEC = RecordCodecBuilder.mapCodec(instance -> singletonFields(instance)
    .and(ResourceLocation.CODEC.fieldOf("tag").forGetter(entry -> entry.tag.location()))
    .and(RANDOM_MATERIAL_CODEC.fieldOf("material").forGetter(entry -> entry.material))
    .apply(instance, (weight, quality, conditions, functions, tagId, material) ->
    new ToolPartLootEntry(TagKey.create(Registries.ITEM, tagId), material, weight, quality, conditions, functions)));
