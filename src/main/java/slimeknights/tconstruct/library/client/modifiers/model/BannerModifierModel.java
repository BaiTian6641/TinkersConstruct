...
public record BannerModifierModel(@Nullable ResourceLocation smallPrefix, @Nullable ResourceLocation largePrefix) implements ModifierModel {
  ...
  public void validate(Function<Material, TextureAtlasSprite> spriteGetter) {
    ...
    if (Config.CLIENT.logMissingModifierTextures.get()) {
      for (ResourceLocation patternId : BannerModule.getKnownPatternIds()) {
        String suffix = MaterialRenderInfo.getSuffix(patternId);
        if (smallPrefix != null) {
    ...
    if (prefix != null) {
      ...
      if (modData.contains(key, CompoundTag.TAG_LIST)) {
        ...
        for (int i = 0; i < list.size(); i++) {
          // patterns are stored as registry IDs so we can survive the removed hash-name API
          CompoundTag tag = list.getCompound(i);
          Holder<BannerPattern> pattern = BannerModule.getPattern(tag.getString(BannerModule.KEY_PATTERN));
          int color = tag.getInt(BannerModule.KEY_COLOR);
          if (pattern != null) {
            TextureAtlasSprite sprite = spriteGetter.apply(ModifierModel.blockAtlas(prefix.withSuffix(MaterialRenderInfo.getSuffix(pattern.value().assetId()))));
