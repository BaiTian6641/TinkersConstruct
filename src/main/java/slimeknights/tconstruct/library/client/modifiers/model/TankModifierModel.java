...
public class TankModifierModel implements ModifierModel {
  ...
  private record CacheKey(Fluid fluid, @Nullable DataComponentPatch componentsPatch, boolean partial) {}
  ...
  public Object getCacheKey(IToolStackView tool, ModifierEntry modifier) {
    ...
    if (!fluid.isEmpty()) {
      return new CacheKey(fluid.getFluid(), fluid.isComponentsPatchEmpty() ? null : fluid.getComponentsPatch(), fluid.getAmount() + tolerance < helper.getCapacity(tool));
    }
    return null;
  }
