...
public record SlotInChargeModule(TinkerDataKey<SlotInCharge> key, @Nullable TagKey<Item> heldTag) implements HookProvider, EquipmentChangeModifierHook {
  ...
  public static boolean isInCharge(@Nullable TinkerDataCapability.Holder capability, TinkerDataKey<SlotInCharge> key, EquipmentSlot slot) {
    TinkerDataCapability.Holder data = capability;
    if (data != null) {
  ...
  public static int getLevel(@Nullable TinkerDataCapability.Holder capability, TinkerDataKey<SlotInCharge> key, EquipmentSlot slot) {
    TinkerDataCapability.Holder data = capability;
    if (data != null) {
      SlotInCharge inCharge = data.get(key);
