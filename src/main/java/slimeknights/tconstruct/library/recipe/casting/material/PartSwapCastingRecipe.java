            ...
            .flatMap(recipe -> {
              ...
              for (ItemStack cast : casts) {
                ...
                if (index < requirements.size() && requirements.get(index).canUseMaterial(output.getId())) {
                  ...
                  CompoundTag displayTag = input.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
                  displayTag.putBoolean(TooltipUtil.KEY_DISPLAY, true);
                  input.set(DataComponents.CUSTOM_DATA, CustomData.of(displayTag));
                  inputs.add(input);
                }
              }
